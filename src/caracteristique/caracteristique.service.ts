...
export class CaracteristiqueService {
	constructor(@InjectModel(Caracteristique.name) private caracteristiqueModel: Model<CaracteristiqueDocument>) {}

	async getAllCaracteristiques(): Promise<Caracteristique[]> {
    	const createdCaracteristique = await this.caracteristiqueModel.find().exec();
	...
	async createCaracteristique(caracteristique: CaracteristiqueDto): Promise<Caracteristique | Error> {
	console.log(caracteristique)
    try {
      return await new this.caracteristiqueModel(caracteristique);
      // console.log(carac)
      // return await carac.save()
    } catch (e) {
